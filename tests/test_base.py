...
def random_cases_for_dates(dates):
    return {'n_cases': np.random.randint(low=5, high=10, size=len(dates)),
...
def test_data_in_the_future():
    ...
    train_data = pd.DataFrame(random_cases_for_dates(dates),
                              index=dates)
    test_data = pd.DataFrame(random_cases_for_dates(future_dates),
                             index=future_dates)
...
def test_fit():
    ...
    train_data = pd.DataFrame({'n_cases': [5, 6],
                               'n_outbreak_cases': [0, 1]},
                              index=dates)
