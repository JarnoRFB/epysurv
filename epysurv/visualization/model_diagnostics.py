...
def ghozzi_score_plot(prediction_result: pd.DataFrame, filename: str):
    ...
    prediction_result['weighted_true_positives'] = (prediction_result.alarm
                                                    * prediction_result.outbreak
                                                    * prediction_result.n_outbreak_cases)

    # Outbreaks that were missed.
    prediction_result['weighted_false_negatives'] = ((1 - prediction_result.alarm)
                                                     * prediction_result.outbreak
                                                     * prediction_result.n_outbreak_cases)
    # Alarms that were falsely raised.
    prediction_result['weighted_false_positives'] = (prediction_result.alarm
    ...
    melted_prediction_result = (
        prediction_result
            ...
            .melt(id_vars=['date', 'county', 'pathogen', 'n_cases', 'n_outbreak_cases', 'outbreak', 'alarm'],
                  var_name='prediction', value_name='weighting')
    )
    ...
    chart = (gg.ggplot(melted_prediction_result, gg.aes(x='date'))
             + gg.geom_bar(prediction_result, gg.aes(x='prediction_result.index', y='n_cases'), fill=case_color,
                           stat='identity')
